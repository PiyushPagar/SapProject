...
//                .imageData(ImageUtils.compressImage(file.getBytes())).build());
    	PaymentRecipetDetails paymentRecipetDetails = new PaymentRecipetDetails();
    	paymentRecipetDetails.setFilename(fileName);
    	paymentRecipetDetails.setFiletype(file.getContentType());
    	paymentRecipetDetails.setUserId(userId);
    	paymentRecipetDetails.setRecieptverifyStatus(ConstantUtil.NotVerified);
    	paymentRecipetDetails.setDateTime(currDateTime);
    	paymentRecipetDetails.setPaymentInstallmentId(installmentNumber);
    	paymentRecipetDetails.setImageData(ImageUtils.compressImage(file.getBytes()));
    	PaymentRecipetDetails imageData =repository.save(paymentRecipetDetails);
    	if (imageData != null) {
